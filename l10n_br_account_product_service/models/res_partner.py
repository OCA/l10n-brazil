...
class AccountFiscalPositionTemplate(models.Model):
    ...
    def generate_fiscal_position(self, cr, uid, chart_temp_id,
        ...
        for position in self.browse(cr, uid, fp_ids, context=context):
            fc_categ = position.fiscal_category_id and\
                position.fiscal_category_id.id
            new_fp = obj_fiscal_position.create(
                cr, uid, {'company_id': company_id,
                          ...
                          'fiscal_category_id': fc_categ})
            for tax in position.tax_ids:
                txtpl = tax_template_ref
                tctpl = tax_code_template_ref
                txsrc = tax.tax_src_id
                tcsrc = tax.tax_code_src_id
                txdest = tax.tax_dest_id
                tcdest = tax.tax_code_dest_id
                obj_tax_fp.create(cr, uid, {
                    'tax_src_id': txsrc and txtpl.get(txsrc.id),
                    'tax_code_src_id': tcsrc and tctpl.get(tcsrc.id),
                    'tax_src_domain': tax.tax_src_domain,
                    'tax_dest_id': txdest and txtpl.get(txdest.id),
                    'tax_code_dest_id': tcdest and tctpl.get(tcdest.id),
                    'position_id': new_fp
                })
