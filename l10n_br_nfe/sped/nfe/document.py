...
class NFe200(FiscalDocument):
    ...
    def _nfe_identification(self, invoice, company, nfe_environment):
        ...
        #
        self.nfe.infNFe.ide.cUF.valor = (
            company.state_id and company.state_id.ibge_code or ""
        )
        self.nfe.infNFe.ide.cNF.valor = ""
        self.nfe.infNFe.ide.natOp.valor = invoice.fiscal_category_id.name[:60] or ""
        self.nfe.infNFe.ide.indPag.valor = (
            invoice.payment_term_id and invoice.payment_term_id.indPag or "0"
        )
        ...
        self.nfe.infNFe.ide.dSaiEnt.valor = (
            datetime.strptime(invoice.date_in_out, "%Y-%m-%d %H:%M:%S").date() or ""
        )
        ...
        if invoice.partner_shipping_id:
            if invoice.partner_id.id != invoice.partner_shipping_id.id:
                if self.nfe.infNFe.ide.tpNF.valor == 0:
                    ...
                    self.nfe.infNFe.retirada.cMun.valor = "{}{}".format(
                    ...
                    self.nfe.infNFe.entrega.cMun.valor = "{}{}".format(
                        invoice.partner_shipping_id.state_id.ibge_code,
                        invoice.partner_shipping_id.l10n_br_city_id.ibge_code,
                    )
    ...
    def _nfe_references(self, inv_related):
        ...
        if inv_related.document_type == "nf":
            ...
            self.nfref.refNF.AAMM.valor = (
                datetime.strptime(inv_related.date, "%Y-%m-%d").strftime("%y%m") or ""
            )
            self.nfref.refNF.CNPJ.valor = punctuation_rm(inv_related.cnpj_cpf)
            self.nfref.refNF.mod.valor = (
                inv_related.fiscal_document_id
                and inv_related.fiscal_document_id.code
                or ""
            )
    ...
    def _emmiter(self, invoice, company):
        ...
        for inscr_est_line in invoice.company_id.partner_id.other_inscr_est_lines:
    ...
    def _receiver(self, invoice, company, nfe_environment):
        ...
        if invoice.partner_id.country_id.id != invoice.company_id.country_id.id:
            ...
            address_invoice_city = normalize(
                "NFKD", unicode(invoice.partner_id.l10n_br_city_id.name or "")
            ).encode("ASCII", "ignore")
    ...
    def _details(self, invoice, invoice_line, index):
        ...
        if invoice_line.product_id:
            ...
            self.det.prod.xProd.valor = normalize(
                "NFKD", unicode(invoice_line.product_id.name[:120] or "")
            ).encode("ASCII", "ignore")
        else:
            self.det.prod.cProd.valor = invoice_line.code or ""
            self.det.prod.xProd.valor = normalize(
                "NFKD", unicode(invoice_line.name[:120] or "")
            ).encode("ASCII", "ignore")
        ...
        self.det.prod.CEST.valor = punctuation_rm(invoice_line.cest_id.code or "")
    ...
    def _encashment_data(self, invoice, move_line, numero_dup):
        ...
        self.dup.dVenc.valor = (
            move_line.date_maturity or invoice.date_due or invoice.date_invoice
        )
    ...
    def _carrier_data(self, invoice):
        ...
        self.nfe.infNFe.transp.modFrete.valor = (
            invoice.incoterm and invoice.incoterm.freight_responsibility or "9"
        )

        if invoice.carrier_id:

            if invoice.carrier_id.partner_id.is_company:
                self.nfe.infNFe.transp.transporta.CNPJ.valor = punctuation_rm(
                    invoice.carrier_id.partner_id.cnpj_cpf or ""
                )
            else:
                self.nfe.infNFe.transp.transporta.CPF.valor = punctuation_rm(
                    invoice.carrier_id.partner_id.cnpj_cpf or ""
                )
    ...
    def get_NFe(self):
        ...
        try:
            ...
            raise UserError(_(u"Biblioteca PySPED não instalada!"))
...
class NFe310(NFe200):
    ...
    def _nfe_identification(self, invoice, company, nfe_environment):
        ...
        super(NFe310, self)._nfe_identification(invoice, company, nfe_environment)
        ...
        self.aut_xml.CNPJ.valor = punctuation_rm(invoice.company_id.accountant_cnpj_cpf)
    ...
    def _export(self, invoice):
        ...
        self.nfe.infNFe.exporta.xLocDespacho.valor = invoice.expedition_location or ""
