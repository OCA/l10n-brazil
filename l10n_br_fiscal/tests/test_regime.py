...
class TestRegime(OperationCatalogCommon):
    ...
    def test_simples_casa_linha_csosn(self):
        ...
        op = f"{MODULE}.fo_remessa_vasilhame"
        ...
        self.assertEqual(defs_lp["ICMS"].cst_id.code, "41")
    ...
    def test_excesso_sublimite_usa_cst(self):
        ...
        line = self._line_for(f"{MODULE}.fo_remessa_vasilhame", company)
    ...
    def test_simbolica_sn_usa_900(self):
        ...
        for xmlid in ("fo_consig_devsimb_line_sn", "fo_baixa_estoque_line_sn"):
            line = self.env.ref(f"{MODULE}.{xmlid}")
            defs = {d.tax_group_id.name: d for d in line.tax_definition_ids}
            self.assertEqual(defs[SN_GROUP].cst_id.code, "900", xmlid)
