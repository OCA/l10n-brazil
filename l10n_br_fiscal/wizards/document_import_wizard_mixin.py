...
class DocumentImportWizard(models.TransientModel):
    ...
    _description = "Wizard Import Document"
    ...
    def _fill_wizard_from_binding(self):
        ...
        3. Detectar Emissor
        4. Detectar Destinatário
        5. Detectar tipo de XML
        6. Procurar Chave
        7. Procurar Documento já existente

        """
        binding = self._parse_file()
        self._detect_binding(binding)
        self._document_key_from_binding(binding)
    ...
    def _parse_file_data(self, file_data):
        try:
            ...
            raise UserError(_("Invalid xml file!")) from e
        return binding

    def _import_edoc(self):
        if not self.document_id:
            binding, self.document_id = self._create_edoc_from_file()
            self.document_id.imported_document = True
    ...
    def _find_fiscal_operation(self, cfop, nat_op, fiscal_operation_type):
        # LEGACY: Esse método não esta sendo usado.
