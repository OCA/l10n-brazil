...
class ImportDeclaration(models.Model):
    ...
    name = fields.Char(string=u"Número da DI", size=10, required=True)

    date_registration = fields.Date(string=u"Data de Registro", required=True)

    exporting_code = fields.Char(string=u"Código do Exportador", required=True, size=60)
    ...
    location = fields.Char(string=u"Local", required=True, size=60)

    date_release = fields.Date(string=u"Data de Liberação", required=True)
    ...
    afrmm_value = fields.Float(
        string=u"Valor da AFRMM", digits=dp.get_precision("Account"), default=0.00
    )
    ...
    thirdparty_cnpj = fields.Char(string=u"CNPJ", size=18)
    ...
    def _onchange_thirdparty_cnpj(self):
        ...
        cpf_cnpj = fiscal.format_cpf_cnpj(self.thirdparty_cnpj, country, True)
