...
class ImportDeclarationLine(models.Model):
    ...
    sequence = fields.Integer(string=u"Sequência", default=1, required=True)
    name = fields.Char(string=u"Adição", size=3, required=True)
    manufacturer_code = fields.Char(
        string=u"Código do Fabricante", size=60, required=True
    )
