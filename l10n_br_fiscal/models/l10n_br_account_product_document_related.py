...
class L10nbrAccountDocumentRelated(models.Model):
    ...
    access_key = fields.Char(string=u"Chave de Acesso", size=44)

    serie = fields.Char(string=u"Série", size=12)

    number = fields.Char(string=u"Número", size=32)
    ...
    cnpj_cpf = fields.Char(string=u"CNPJ/CPF", size=18)
    ...
    def _check_ie(self):
        ...
        if not check_ie:
            raise UserError(_(u"Inscrição Estadual do documento fiscal inválida!"))
    ...
    def _onchange_invoice_related_id(self):
        ...
        if related.fiscal_document_id.code in ("01", "04"):
            ...
            if related.issuer == "0":
                self.serie = (
                    related.document_serie_id
                    and related.document_serie_id.code
                    or False
                )
            else:
            ...
            self.fiscal_document_id = (
                related.fiscal_document_id and related.fiscal_document_id.id or False
            )
