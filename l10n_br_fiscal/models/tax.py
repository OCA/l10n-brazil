        ...
        for tax in self:
            ...
            try:
                ...
                compute_method = getattr(
                    self, '_compute_%s' % tax.tax_domain)
                taxes[tax.tax_domain].update(compute_method(
                    tax, taxes[tax.tax_domain], **kwargs))
            except AttributeError:
                taxes[tax.tax_domain].update(tax._compute_generic(
                    tax, taxes[tax.tax_domain], **kwargs))
