...
class AccountFiscalPosition(models.Model):
    ...
    def _map_tax(self, taxes, product=None, partner=None):
        ...
        if self.company_id and context.get("type_tax_use") in ("sale", "all"):
                ...
                for tax_def in state_tax_def:
                    if tax_def.tax_id:
                        ...
                        if (not fc and not tax_def.cest_id) or (
                            fc == product_fc or tax_def.cest_id == product.cest_id
                        ):
                            taxes |= tax_def.tax_id
        ...
        for tax in taxes:
            for mapping in self.tax_ids:
                if (
                    mapping.tax_src_id.id == tax.id
                    or mapping.tax_dest_id == tax
                    or mapping.tax_group_id.id == tax.tax_group_id.id
                ):
                    if mapping.tax_dest_id.id or tax.tax_code_id.id:
                        if (
                            mapping.fiscal_classification_id.id
                        ...
                        if (
                            mapping.fiscal_classification_id.id
                            == product.fiscal_classification_id.id
                            and mapping.origin == product.origin
                        ):
                            map_taxes_origin_ncm |= mapping
                        if (
                            not mapping.origin
                            and not mapping.fiscal_classification_id
                            and not mapping.cest_id
                        ):
                            map_taxes |= mapping
            else:
...
class AccountFiscalPositionTax(models.Model):
    ...
    origin = fields.Selection(selection=PRODUCT_ORIGIN, string=u"Origem")

    cest_id = fields.Many2one(
        comodel_name="l10n_br_account_product.cest", string=u"CEST"
    )
    ...
    tax_icms_relief_id = fields.Many2one(
        comodel_name="l10n_br_account_product.icms_relief", string=u"Desoneração ICMS"
    )

    cst_id = fields.Many2one(
        comodel_name="l10n_br_account_product.cst", string=u"CST", required=False
    )

    @api.onchange("tax_dest_id", "tax_group_id", "position_id")
    def _onchange_tax_group(self):
        onchange = super(AccountFiscalPositionTax, self)._onchange_tax_group()

        if onchange:
            onchange["domain"].update({"cst_id": onchange["domain"]["tax_dest_id"]})
        return onchange.update(
            {"readonly": {"tax_icms_relief_id": [("tax_dest_id.domain", "=", "icms")]}}
        )
