...
class MDFe(models.Model):
    ...
    document_key = fields.Char(string="Access Key", size=44,)
    serie = fields.Char(string="Serie", size=3, index=True,)
    document_number = fields.Float(
        string="Document Number", index=True, digits=(18, 0),
    )
    ...
    document_value = fields.Float(
        string="Document Total Value", readonly=True, digits=(18, 2),
    )
    ...
    supplier = fields.Char(string="Supplier", size=60, index=True,)

    emission_datetime = fields.Datetime(
        string="Emission Date", index=True, default=fields.Datetime.now,
    )
    inclusion_datetime = fields.Datetime(
        string="Inclusion Date", index=True, default=fields.Datetime.now,
    )
    ...
    document_state = fields.Selection(
        string="Document State", selection=SITUACAO_NFE, index=True,
    )

    state = fields.Selection(
        string="Manifestation State", selection=SITUACAO_MANIFESTACAO, index=True,
    )
