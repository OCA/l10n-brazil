...
class TaxICMSRegulation(models.Model):
    ...
    icms_ids = fields.One2many(
        comodel_name="l10n_br_fiscal.tax.icms", inverse_name="regulation_id"
    )
...
class TaxICMS(models.Model):
    ...
    regulation_id = fields.Many2one(
        comodel_name="l10n_br_fiscal.tax.icms.regulation", string="ICMS Regulation"
    )

    name = fields.Text(string="Name", required=True, index=True)

    state_to_id = fields.Many2one(comodel_name="res.country.state", string="State To")

    state_from_id = fields.Many2one(comodel_name="res.country.state", string="State To")
