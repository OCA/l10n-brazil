...
class ConsultaDFe(models.Model):
    ...
    def busca_documentos(self):
        for consulta in self:
            ...
            # uu_obj.write({
            # 'ultimo_nsu': processo.resposta.ultNSU.valor,
            ...
            # cron_id = self.pool.get('ir.cron').search(cr, 1, [
            # ('function', '=', 'acao_demorada_busca_documentos')])
            ...
            # def acao_demorada_busca_documentos(
            # self, cr, uid, ids=[], context={}):
            # if not ids:
            # ids = self.pool.get('sped.ultimo_nsu').search(cr, uid, [])
            # self.pool.get('sped.ultimo_nsu').busca_documentos(
            #   cr, uid, ids, context)
