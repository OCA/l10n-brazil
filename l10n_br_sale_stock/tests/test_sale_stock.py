...
class TestSaleStock(TestBrPickingInvoicingCommon):
    ...
    def _setup_test_sale_orders(cls):
        ...
        if team:
            for val in [cls._sale_1_vals, cls._sale_2_vals,
                         cls._sale_3_vals, cls._sale_4_vals]:
                val["team_id"] = team.id
    ...
    def test_synchronize_sale_partner_shipping_in_stock_picking(self):
        ...
        """
        so = self._create_sale_order(self._sale_1_vals, [
            {"product_id": self.product_prod.id, "product_uom_qty": 2,
             "name": self.product_prod.name, "price_unit": 100.0,
             "fiscal_operation_id": self.fo_venda.id},
        ])
        so.action_confirm()
