...
class L10nBRZipTest(TransactionCase):
    ...
    def test_search_zip_code_company(self):
        ...
        self.assertEquals(
            self.company.district, 'Bela Vista',
            'Error in method zip_search to mapping field district.')
        self.assertEquals(
            self.company.street, 'Avenida Avenida Paulista 1842',
            'Error in method zip_search to mapping field street.')
        self.assertEquals(
            self.company.city_id.name, u'São Paulo',
    ...
    def test_return_two_results_zip_search(self):
        ...
        result = self.company_1.zip_search()
        obj_zip_search = self.env[
            'l10n_br.zip.search'].browse(result.get('res_id'))
        ...
        self.assertEquals(
            obj_zip_search.street, 'paulista',
            'It should return the correct street')
        self.assertEquals(
            obj_zip_search.state_id.id, self.env.ref('base.state_br_sp').id,
            'It should return the correct state')
        self.assertEquals(
            len(obj_zip_search.zip_ids), 2,
            'It should return two Zip Codes.')
        ...
        self.assertEquals(
            len(obj_zip_search.zip_ids), 1,
            'It should return one Zip Codes.')
    ...
    def test_return_pycep_correios(self):
        ...
        self.company.zip_search()
        self.assertEquals(
            self.company.district, 'Paraíso',
            'Error in method zip_search with PyCEP-Correios'
            'to mapping field district.')
        self.assertEquals(
            self.company.street, 'Rua Coronel Oscar Porto',
            'Error in method zip_search with PyCEP-Correios'
