...
class AccountInvoice(models.Model):
    ...
    fiscal_document_id = fields.Many2one(
        ...
        default=lambda self: self.env.ref("l10n_br_fiscal.fiscal_document_dummy"),
    )
...
if False:  # TODO
    ...
    def _compute_amount(self):
        for record in self:
            ...
            record.amount_tax = sum(
                t.amount for t in record.tax_line_ids if not t.tax_id.tax_discount
            )
            ...
            if (
                ...
                and record.currency_id != record.company_id.currency_id
            ):
        ...
        for record in self:
            ...
            for line in record.move_id.line_ids:
                if (
                    ...
                    and record.journal_id.revenue_expense
                ):
                    lines |= line
    ...
    @api.multi
    def name_get(self):
        return [(r.id, u"{}".format(r.number or "")) for r in self]

    _sql_constraints = [
        (
            "number_uniq",
            "unique(number, company_id, journal_id,\
         type, partner_id)",
    ...
    def finalize_invoice_move_lines(self, move_lines):
        ...
        count = 1
        total = len([x for x in move_lines if x[2]["account_id"] == self.account_id.id])
            ...
            for move_line in move_lines:
                if move_line[2]["debit"] or move_line[2]["credit"]:
                    if move_line[2]["account_id"] == self.account_id.id:
                        move_line[2]["name"] = "{}/{}-{}".format(number, count, total)
                        count += 1
