    ...
    def compute_refund(self, mode="refund"):
        ...
        for send_invoice in inv_obj.browse(context.get("active_ids")):
            ...
            for invoice in inv_obj.browse(invoice_ids):
                ...
                if (
                    not self.force_fiscal_category_id
                    and not invoice.fiscal_category_id.refund_fiscal_category_id
                ):
                    raise UserError(
                        _(
                            "Categoria Fiscal: %s não possui uma "
                ...
                invoice.fiscal_category_id = (
                    self.force_fiscal_category_id.id
                    or invoice.fiscal_category_id.refund_fiscal_category_id.id
                )
                ...
                for line in invoice.invoice_line_ids:
                    if (
                        not self.force_fiscal_category_id
                        and not line.fiscal_category_id.refund_fiscal_category_id
                    ):
                        raise UserError(
                            _(
                                u"Categoria Fiscal: %s não possui"
                    ...
                    line.fiscal_category_id = (
                        self.force_fiscal_category_id.id
                        or line.fiscal_category_id.refund_fiscal_category_id.id
                    )
