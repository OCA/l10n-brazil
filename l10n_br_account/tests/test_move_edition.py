...
class TestMoveEdition(TransactionCase):
    ...
    def test_out_fiscal_invoice(self):
        ...
        with move_form.invoice_line_ids.new() as line_form:
            ...
            self.assertEqual(
                line_form.icms_tax_id.id, self.ref("l10n_br_fiscal.tax_icms_7")
            )
            self.assertEqual(
                line_form.ipi_tax_id.id, self.ref("l10n_br_fiscal.tax_ipi_nt")
            )
            ...
            self.assertEqual(
                line_form.ipi_tax_id.id, self.ref("l10n_br_fiscal.tax_ipi_5")
            )
