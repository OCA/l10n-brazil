...
class TestFiscalDocumentNFSePaulistana(TransactionCase):
    ...
    def setUp(self):
        ...
        self.company_paulistana = self.env.ref(
            'l10n_br_fiscal.empresa_simples_nacional')

        self.company_paulistana.processador_edoc = 'erpbrasil_edoc'
        self.company_paulistana.partner_id.inscr_mun = '35172'
        self.company_paulistana.partner_id.inscr_est = ''
        self.company_paulistana.partner_id.state_id = self.env.ref(
            'base.state_br_mg')
        self.company_paulistana.partner_id.city_id = self.env.ref(
            'l10n_br_base.city_3132404')
        self.company_paulistana.icms_regulation_id = self.env.ref(
            'l10n_br_fiscal.tax_icms_regulation').id
        self.company_paulistana.city_taxation_code_id = self.env.ref(
            'l10n_br_fiscal.city_taxation_code_itajuba').id
        self.company_paulistana.document_type_id = self.env.ref(
        ...
        self.company_paulistana.certificate_nfe_id = cert
        self.nfse_same_state.company_id = self.company_paulistana.id
    ...
    def test_nfse_paulistana(self):
        ...
        xml_path = os.path.join(
            l10n_br_nfse_paulistana.__path__[0], 'tests', 'nfse',
            '001_50_nfse.xml')
