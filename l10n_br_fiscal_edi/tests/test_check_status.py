...
class TestCheckStatus(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.with_key = cls._create_document(
            "35260912345678000195550020000000061765922237"
        )
    ...
    def test_an_authorized_document_is_still_asked_about(self):
        authorized = self._create_document(
            "35200159594315000157550010000000012062777161",
            state=DOCUMENT_STATE_AUTHORIZED,
        )
        with patch(f"{FROM_THE_DOCUMENT}._document_status", autospec=True) as consulted:
            authorized.action_check_status()
    ...
    def test_only_the_limit_is_asked_about_the_rest_is_named(self):
        second = self._create_document("35260912345678000195550020000000061765922233")
        with (
