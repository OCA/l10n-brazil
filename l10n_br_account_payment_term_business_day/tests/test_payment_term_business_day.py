...
class TestPaymentTermBusinessDay(TransactionCase):
    ...
    def test_municipal_holiday_only_moves_its_own_calendar(self):
        ...
        city_calendar = self.env["resource.calendar"].create(
            {"name": "Cidade Exemplo", "parent_id": self.calendar.id}
        )
        self.env["resource.calendar.leaves"].create(
            {
                ...
                "leave_type": "F",
                "calendar_id": city_calendar.id,
            }
        )
        local = self._term("previous", calendar=city_calendar)
