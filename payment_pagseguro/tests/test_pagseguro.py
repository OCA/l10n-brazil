...
class PagseguroCommon(PaymentAcquirerCommon):
    def setUp(self):
        ...
        self.currency_brl = self.env["res.currency"].search(
            [("name", "=", "BRL")], limit=1
        )
...
class PagseguroTest(PagseguroCommon):
    def test_10_pagseguro_s2s_capture(self):
        self.assertEqual(
            self.pagseguro.environment, "test", "test without test environment"
        )
        ...
        self.assertEqual(
            transaction.state, "authorized", "transaction state should be authorized"
        )

        time.sleep(3)
        transaction.action_capture()
        self.assertEqual(transaction.state, "done", "transaction state should be done")
