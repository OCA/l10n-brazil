...
class RodoOs(models.AbstractModel):
    ...
    cte30_NroRegEstadual = fields.Char(
    ...
    cte30_veic = fields.Many2one(comodel_name="cte.30.veic", string="Dados do Veículo")

    cte30_infFretamento = fields.Many2one(
        comodel_name="cte.30.inffretamento",
...
class Veic(models.AbstractModel):
    ...
    cte30_placa = fields.Char(string="Placa do veículo", xsd_required=True)

    cte30_RENAVAM = fields.Char(string="RENAVAM do veículo")

    cte30_prop = fields.Many2one(
        comodel_name="cte.30.prop",
...
class Prop(models.AbstractModel):
    ...
    cte30_IE = fields.Char(string="Inscrição Estadual")

    cte30_UF = fields.Selection(TUF, string="UF", xsd_type="TUf")

    cte30_tpProp = fields.Selection(
        PROP_TPPROP,
...
class InfFretamento(models.AbstractModel):
    ...
    _description = textwrap.dedent("    %s" % (__doc__,))
    _name = "cte.30.inffretamento"
    _inherit = "spec.mixin.cte"
    _binding_type = "RodoOs.InfFretamento"

    cte30_tpFretamento = fields.Selection(
        INFFRETAMENTO_TPFRETAMENTO,
    ...
    cte30_dhViagem = fields.Datetime(
