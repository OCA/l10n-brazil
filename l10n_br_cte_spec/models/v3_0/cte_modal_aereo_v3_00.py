...
class Aereo(models.AbstractModel):
    ...
    cte30_natCarga = fields.Many2one(
        comodel_name="cte.30.natcarga", string="Natureza da carga", xsd_required=True
    )

    cte30_tarifa = fields.Many2one(
        comodel_name="cte.30.tarifa", string="Informações de tarifa", xsd_required=True
    )

    cte30_peri = fields.One2many(
        "cte.30.peri",
        "cte30_peri_aereo_id",
...
class NatCarga(models.AbstractModel):
    "Natureza da carga"
    _description = textwrap.dedent("    %s" % (__doc__,))
    _name = "cte.30.natcarga"
    _inherit = "spec.mixin.cte"
    _binding_type = "Aereo.NatCarga"

    cte30_xDime = fields.Char(
        string="Dimensão",
        help=(
...
class Peri(models.AbstractModel):
    ...
    cte30_peri_aereo_id = fields.Many2one(
        comodel_name="cte.30.aereo", xsd_implicit=True, ondelete="cascade"
    )
    cte30_nONU = fields.Char(
    ...
    cte30_infTotAP = fields.Many2one(
        comodel_name="cte.30.inftotap",
...
class InfTotAp(models.AbstractModel):
    ...
    _description = textwrap.dedent("    %s" % (__doc__,))
    _name = "cte.30.inftotap"
    _inherit = "spec.mixin.cte"
    _binding_type = "Aereo.Peri.InfTotAp"

    cte30_qTotProd = fields.Float(
