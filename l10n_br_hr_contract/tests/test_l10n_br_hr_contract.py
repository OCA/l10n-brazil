...
class TestL10nBrContract(TransactionCase):
    ...
    def test_admission_type(self):
        admission_type = self.env['hr.contract.admission.type'].search([])[0].\
            name_get()[0][1]
        self.assertEqual(admission_type, '1 - Admissão',
                         'The admission type name by name_get is not valid, '
                         'expected \'code - name\'')

    def test_labor_regime(self):
        labor_regime = self.env['hr.contract.labor.regime'].search([])[0].\
            name_get()[0][1]
        self.assertEqual(labor_regime, 'CLT - Consolidação das Leis de '
                                       'Trabalho',
                         'The labor regime name by name_get is not valid,'
                         ' expected \'code - name\'')

    def test_labor_bond_type(self):
        labor_bond = self.env['hr.contract.labor.bond.type'].search([])[0].\
            name_get()[0][1]
        self.assertEqual(labor_bond, '10 - Trabalhador urbano vinculado a'
    ...
    def test_resignation_cause(self):
        resignation_cause = self.env['hr.contract.resignation.cause'].\
            search([])[0].name_get()[0][1]
        self.assertEqual(resignation_cause, '10 - Rescisão de contrato de'
