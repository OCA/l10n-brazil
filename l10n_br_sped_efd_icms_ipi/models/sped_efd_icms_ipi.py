...
def fiscal_obs_code(text):
    """Deterministic 6-char COD_OBS for a fiscal observation text.

    C195 references a 0460 register by COD_OBS. Deriving the code from the
    text keeps 0460 and C195 consistent without extra master data; the full
    text is still registered in 0460.TXT so the code stays readable.
    """
...
class Registro0460(models.Model):
    ...
    @api.model
    def _odoo_query(self, parent_record, declaration):
        # Register each distinct fiscal observation text carried by the
        # documents of this declaration (imported from <infAdic>/<obsFisco>
        # into manual_fiscal_additional_data). C195 references these by the
        # same deterministic COD_OBS.
        query = """
            SELECT DISTINCT manual_fiscal_additional_data AS txt
            FROM l10n_br_fiscal_document
            WHERE id = ANY(%s)
              AND manual_fiscal_additional_data IS NOT NULL
              AND btrim(manual_fiscal_additional_data) != ''
            ORDER BY txt
        """
        return query, [declaration.fiscal_document_ids.ids]

    @api.model
    def _map_from_odoo(self, record, parent_record, declaration, index=0):
        return {
            "COD_OBS": fiscal_obs_code(record["txt"]),
            "TXT": record["txt"],
        }
...
class RegistroC195(models.Model):
    ...
    @api.model
    def _odoo_query(self, parent_record, declaration):
        # parent_record is the C100 fiscal document. Emit one C195 observation
        # per document carrying a fiscal observation text (from the XML
        # <infAdic> captured at import into manual_fiscal_additional_data).
        query = """
            SELECT id AS document_id, manual_fiscal_additional_data AS txt
            FROM l10n_br_fiscal_document
            WHERE id = %s
              AND manual_fiscal_additional_data IS NOT NULL
              AND btrim(manual_fiscal_additional_data) != ''
...
class RegistroC197(models.Model):
    ...
    @api.model
    def _odoo_query(self, parent_record, declaration):
        # parent_record is the C195 dict ({document_id, txt}). Declare one C197
        # per line of that document carrying an adjustment: ICMS desoneração
        # (relief) or a fiscal benefit (cBenef) declared by the supplier.
        query = """
            SELECT
                fdl.id AS line_id,
                COALESCE(
                ...
                COALESCE(pt.default_code, '') AS cod_item,
                fdl.icms_base AS vl_bc_icms,
                fdl.icms_percent AS aliq_icms,
                fdl.icms_value AS vl_icms,
            ...
            WHERE fdl.document_id = %s
              AND (COALESCE(fdl.icms_relief_value, 0) > 0
                   OR COALESCE(fdl.icms_tax_benefit_code, '') != ''
                   OR COALESCE(fdl.partner_icms_tax_benefit_code, '') != ''
                   OR fdl.icms_relief_id IS NOT NULL)
        """
        return query, [parent_record["document_id"]]

    @api.model
    def _map_from_odoo(self, record, parent_record, declaration, index=0):
        return {
            "COD_AJ": record["cod_aj"] or "",
            "DESCR_COMPL_AJ": record["descr"] or "",
            "COD_ITEM": record["cod_item"] or "",
            "VL_BC_ICMS": record["vl_bc_icms"] or 0.0,
            "ALIQ_ICMS": record["aliq_icms"] or 0.0,
            "VL_ICMS": record["vl_icms"] or 0.0,
            "VL_OUTROS": record["vl_outros"] or 0.0,
        }
