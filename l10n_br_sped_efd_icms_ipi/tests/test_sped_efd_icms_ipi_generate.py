...
class SpedEfdIcmsIpiGenerateTest(common.TransactionCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
    ...
    def test_generate_efd_icms_ipi(self):
        if not self.company:
        ...
        model = self.env["l10n_br_sped.efd_icms_ipi.0000"].with_company(self.company)
        ...
        # documents, product table, analytic record and ICMS apuration
        self.assertIn("\n|C100|", text)
        self.assertIn("\n|0200|", text)
        self.assertIn("\n|C190|", text)
        self.assertIn("\n|E110|", text)
