...
class TestL10nBrMisReport(AccountTestInvoicingCommon):
    ...
    # Pin the core generic chart for the test companies: with the default
    # resolution the full CI database would pick br_oca and try to install
    # l10n_br_account in the middle of the tests, which is forbidden.
    chart_template = "generic_coa"

    @classmethod
    def setUpClass(cls):
        # Odoo 18.0 removed the chart_template_ref argument from
        # AccountTestInvoicingCommon.setUpClass.
        super().setUpClass()
        ...
        def account(code, name, account_type, tag, result=False):
            ...
            tags = [(4, cls.env.ref(f"l10n_br_coa.{tag}").id)]
            if result:
            ...
            return cls.env["account.account"].create(
                {
                    ...
                    # Odoo 18.0: account.account is company-agnostic
                    "tag_ids": tags,
                }
            )
...
class TestL10nBrMisReportPeriods(AccountTestInvoicingCommon):
    ...
    # See TestL10nBrMisReport: the generic chart avoids the br_oca
    # module-install cascade on the full CI database.
    chart_template = "generic_coa"

    def _period(self, xmlid):
        return self.env.ref(f"l10n_br_mis_report.{xmlid}")
