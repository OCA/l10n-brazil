...
class TestCustomerNFe(TransactionCase):
    ...
    def test_customer_nfe_same_state(self):
        ...
        self.assertEquals(
            self.invoice_same_state.state, 'draft',
        ...
        for line in self.invoice_same_state.invoice_line_ids:
            ...
            self.assertEquals(
                line.icms_percent_reduction, 0.0,
            ...
            self.assertEquals(
                line.icms_st_percent_reduction, 0.0,
                "ICMS ST Percent Reduction is not 0.0")

            # Check IPI
            self.assertEquals(
                line.ipi_cst_id.code, '50', "IPI CODE is not 50")
            self.assertEquals(
            ...
            # Check PIS
            self.assertEquals(
                line.pis_cst_id.code, '01', "PIS CODE is not 01")
            self.assertEquals(
                line.pis_base, 1000.0, "PIS BASE is not 1000.0")
            self.assertEquals(
                line.pis_value, 6.50, "PIS Value is not 6.50 ")
            self.assertEquals(
    ...
    def test_customer_invoice_other_costs(self):
        ...
        for line in self.invoice_other_costs.invoice_line_ids:
            ...
            self.assertEquals(
                ...
                "Freight value is not 100.0")
            self.assertEquals(
                line.insurance_value, 10.0,
                "Insurance value is not 10.0")
            self.assertEquals(
                line.other_costs_value, 10.0,
                "Other Costs value is not 10.0")
        self.invoice_other_costs.with_context(
            {'fiscal_document_code': '55'}).action_invoice_open()
        self.assertEquals(
            self.invoice_other_costs.amount_freight, 100.0,
            "Amount Freight is not 100.0")
        self.assertEquals(
            self.invoice_other_costs.amount_insurance, 10.0,
            "Amount Insurance is not 10.0")
        self.assertEquals(
            self.invoice_other_costs.amount_costs, 10.0,
    ...
    def test_customer_invoice_difal(self):
        ...
        for line in self.invoice_difal.invoice_line_ids:
            ...
            self.assertEquals(
                line.icms_dest_percent, 0.18,
                "ICMS Dest Percent is not 0.18 ")
            self.assertEquals(
                line.icms_origin_percent, 12.0,
                "ICMS Origin Percent is not 12.0% ")
            self.assertEquals(
                line.icms_part_percent, 1,
                "ICMS Origin Percent is not 1.0% ")
            self.assertEquals(
