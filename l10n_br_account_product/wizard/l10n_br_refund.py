...
class account_invoice_refund(orm.TransientModel):
    ...
    def compute_refund(self, cr, uid, ids, mode='refund', context=None):
        ...
        if not context:
            context = {}
        for send_invoice in inv_obj.browse(cr, uid, context.get('active_ids'),
                                           context):
            fiscal_category_id = (send_invoice.fiscal_category_id
                                  .refund_fiscal_category_id.id)
            res = (super(account_invoice_refund, self).
                   compute_refund(cr, uid, ids, mode, context))
            domain = res['domain']
            ...
            for invoice in inv_obj.browse(cr, uid, invoice_ids,
                ...
                if invoice.partner_bank_id:
                    ...
                    bank = False
                onchange = inv_obj.onchange_partner_id(cr,
                                                       uid,
                                                       [invoice.id],
                                                       'out_refund',
                ...
                for idx, send_line in enumerate(send_invoice.invoice_line):
                    line_fiscal_category_id = (send_line.fiscal_category_id
                                               .refund_fiscal_category_id.id)
                    line_onchange = line_obj.onchange_fiscal_category_id(
                        cr, uid, ids, invoice.partner_id.id,
                        invoice.company_id.id, send_line.product_id.id,
                        line_fiscal_category_id, send_line.account_id.id,
                        context)
                    line_onchange['value']['fiscal_category_id'] = \
                        line_fiscal_category_id

                    line_obj.write(cr, uid, line_ids[idx],
                                   line_onchange['value'], context)
                inv_obj.write(cr, uid, [invoice.id],
                              onchange['value'], context=context)
            return res

    def fields_view_get(self, cr, uid, view_id=None, view_type='form',
                        context=None, toolbar=False, submenu=False):
        res = (super(account_invoice_refund, self)
               .fields_view_get(cr, uid, view_id, view_type, context,
                                toolbar, submenu))
        if not context:
            context = {}
