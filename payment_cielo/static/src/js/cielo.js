odoo.define('payment_cielo.cielo', function(require) {
    "use strict";
    ...
    var _t = core._t;
    var qweb = core.qweb;
    ajax.loadXML('/payment_cielo/static/src/xml/cielo_templates.xml', qweb);
    ...
    function getStripeHandler()
    {
        ...
        var handler = cieloHandler = CieloCheckout.configure({
            key: $("input[name='cielo_key']").val(),
            image: $("input[name='cielo_image']").val(),
            locale: 'auto',
            token: function(token, args) {
                ...
                ajax.jsonRpc("/payment/cielo/create_charge", 'call', {
                    ...
                    acquirer_id: $("#acquirer_cielo").val(),
                    ...
                    var wizard = $(qweb.render('cielo.error', {'msg': msg || _t('Payment error')}));
    ...
    var observer = new MutationObserver(function(mutations, observer) {
        for(var i=0; i<mutations.length; ++i) {
            for(var j=0; j<mutations[i].addedNodes.length; ++j) {
                if(mutations[i].addedNodes[j].tagName.toLowerCase() === "form" && mutations[i].addedNodes[j].getAttribute('provider') == 'cielo') {
                    display_cielo_form($(mutations[i].addedNodes[j]));
                }
            }
        }
    });

    function display_cielo_form(provider_form) {
        ...
        var acquirer_id = parseInt(provider_form.find('#acquirer_cielo').val());
        ...
        ajax.jsonRpc(payment_tx_url, 'call', {
            ...
            var $pay_cielo = $('#pay_cielo').detach();
    ...
    $.getScript("https://checkout.cielo.com/checkout.js", function(data, textStatus, jqxhr) {
        observer.observe(document.body, {childList: true});
        display_cielo_form($('form[provider="cielo"]'));
    });
});
