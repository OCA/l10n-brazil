...
class TestCpfCnpj(TestCnpjCommon):
    ...
    def test_cpfcnpj_phones_missing_fields(self):
        ...
        """
        phone = self._webservice._cpfcnpj_get_phones(
            {
                "telefones": [
    ...
    def test_cpfcnpj_phones_all_incomplete(self):
        """When no item is complete, phone stays False."""
        phone = self._webservice._cpfcnpj_get_phones(
    ...
    def test_cpfcnpj_phones_without_list(self):
        """A payload without telefones (or with null) yields False."""
        self.assertFalse(self._webservice._cpfcnpj_get_phones({}))
        self.assertFalse(self._webservice._cpfcnpj_get_phones({"telefones": None}))
    ...
    def test_cpfcnpj_qsa_skips_contact_of_other_company(self):
        ...
        # company itself, which has no parent, and move it instead of skipping
        # the contact. Skip the sync so only the contact carries the vat, as in
        # 18.0 where this test was written.
        self.model.with_context(_partners_skip_fields_sync=True).create(
            {
