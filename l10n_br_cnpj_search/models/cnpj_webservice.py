...
class CNPJWebservice(models.AbstractModel):
    ...
    def _cpfcnpj_import_data(self, data):
        ...
        phone = self._cpfcnpj_get_phones(data)
    ...
    def _cpfcnpj_get_phones(self, data):
        """Return the first complete phone number found in the response.

        Items missing ddd or numero are skipped. Odoo 19.0 dropped
        res.partner.mobile, so like the sibling providers only the first valid
        number is used; the remaining numbers are ignored.
        """
