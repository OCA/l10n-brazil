...
def clean_demo_cnab_config(env):
    ...
    if is_demo:
        for cnab_config in env["l10n_br_cnab.config"].search([]):
            cnab_config.own_number_sequence_id = False
            cnab_config.cnab_sequence_id = False
...
def migrate(env, version):
    if not version:
        return
    is_cnab_config_already_exist = env["l10n_br_cnab.config"].search([])
