...
def update_cnab_config_fee_code(env):
    """
    Atualiza o Código de Multa de CHAR para Objeto/l10n_br_cnab.code
    """

    env.cr.execute(
        """
        SELECT id, boleto_fee_code FROM l10n_br_cnab_config WHERE
        boleto_fee_code IS NOT NULL;
        """
    )
    for row in env.cr.fetchall():
        cnab_config = env["l10n_br_cnab.config"].browse(row[0])
        if row[1]:
            fee_obj = env["l10n_br_cnab.code"].search(
                [
            ...
            if fee_obj:
                cnab_config.boleto_fee_code_id = fee_obj
            else:
                ...
                cnab_config.boleto_fee_code_id = code_not_found
...
def migrate(env, version):
    if not version:
        return
    update_cnab_config_fee_code(env)
