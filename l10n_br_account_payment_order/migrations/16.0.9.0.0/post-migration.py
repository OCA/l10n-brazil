# Copyright (C) 2025-Today - Akretion - Magno Costa <magno.costa@akretion.com.br>
# License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).

from openupgradelib import openupgrade


def update_cnab_config_interest_code(env):
    """
    Atualiza o Código de Mora de CHAR para Objeto/l10n_br_cnab.code
    """

    env.cr.execute(
        """
        SELECT id, boleto_interest_code FROM l10n_br_cnab_config WHERE
        boleto_interest_code IS NOT NULL;
        """
    )
    for row in env.cr.fetchall():
        cnab_config = env["l10n_br_cnab.config"].browse(row[0])
        if row[1]:
            interest_obj = env["l10n_br_cnab.code"].search(
                [
                    ("bank_ids", "in", cnab_config.bank_id.ids),
                    ("payment_method_ids", "in", cnab_config.payment_method_id.ids),
                    ("code_type", "=", "interest_code"),
                    ("code", "=", row[1]),
                ]
            )
            if interest_obj:
                cnab_config.boleto_interest_code_id = interest_obj
            else:
                ...
                cnab_config.boleto_interest_code_id = code_not_found
...
def migrate(env, version):
    if not version:
        return
    update_cnab_config_interest_code(env)
