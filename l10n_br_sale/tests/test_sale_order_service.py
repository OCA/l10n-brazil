...
class TestSaleOrderService(common.TransactionCase):
    ...
    def test_sale_order_service(self):
        ...
        so_id = self.sale_order.create(
            cr, uid, vals={"partner_id": self.partner_id}, context=context
        )

        obj_sale_order = self.sale_order.browse(cr, uid, [so_id], context=context)
        ...
        # Create Invoice
        self.sale_order.manual_invoice(cr, uid, [so_id], context=context)
