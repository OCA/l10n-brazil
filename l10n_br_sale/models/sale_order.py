...
class SaleOrder(models.Model):
    ...
    def fields_view_get(
        ...
        if view_type == 'form':
            form_view_id = self.env['ir.model.data'].xmlid_to_res_id('l10n_br_sale.l10n_br_sale_order_form')
            if order_view.get('view_id') == form_view_id:
                modifiers = {}
                dom = etree.fromstring(res['arch'])
                for node in dom.xpath("//field[@name='order_line']//form//page[@name='fiscal_taxes']"):
                    brazil_country_id = self.env['ir.model.data'].xmlid_to_res_id('base.br')
                    modifiers['invisible'] = [('country_id', '!=', brazil_country_id)]
