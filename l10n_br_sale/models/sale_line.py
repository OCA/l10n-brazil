...
class SaleOrderLine(models.Model):
    ...
    def _fiscal_position_map(self, **kwargs):
        ...
        product_fc_id = fp_rule_obj.with_context(context).product_fiscal_category_map(
        ...
        result["value"]["fiscal_category_id"] = kwargs.get("fiscal_category_id")

        obj_fiscal_position = fp_rule_obj.with_context(context).apply_fiscal_mapping(
            **kwargs
        )
    ...
    def onchange_fiscal(self):
        for record in self:
            if (
                ...
                and record.order_id.partner_id
                ...
                kwargs = {
                    ...
                    or record.order_id.fiscal_category_id,
    ...
    def _prepare_invoice_line(self, qty):
        ...
        result["fiscal_category_id"] = (
            self.fiscal_category_id.id or self.order_id.fiscal_category_id.id or False
        )
        result["fiscal_position_id"] = (
            self.fiscal_position_id.id or self.order_id.fiscal_position_id.id or False
        )
        return result
