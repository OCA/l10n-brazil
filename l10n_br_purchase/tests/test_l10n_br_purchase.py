...
class TestL10nBRPurchase(common.TransactionCase):
    def setUp(self):
        ...
        self.fiscal_categ_compras = self.env["l10n_br_account.fiscal.category"].browse(
            self.ref("l10n_br_purchase.demo_fiscal_category-compras")
        )
        self.fiscal_categ_compras_st = self.env[
            "l10n_br_account.fiscal.category"
        ].browse(self.ref("l10n_br_purchase.demo_fiscal_category-compras_sp_st"))
    ...
    def test_l10n_br_purchase_order(self):
        ...
        self.assertEquals(
            self.purchase_order_1.fiscal_position_id.name,
            "Compra SP c/ ST",
            ...
            for tax in line.taxes_id:
                if tax.tax_group_id.name == "IPI":
                    self.assertEquals(
                        tax.name,
                        u"IPI Entrada 2% *",
                        u"Error to mapping correct TAX (" u" IPI Entrada 2% *).",
                    )
        self.purchase_order_1.button_confirm()

        self.assertEquals(
            self.purchase_order_1.state, "purchase", "Error to confirm Purchase Order."
        )
        ...
        pick.action_done()
        self.assertEquals(
            self.purchase_order_1.invoice_status,
            "to invoice",
        ...
        wizard_obj = self.stock_invoice_onshipping.with_context(
            active_ids=[pick.id], active_model=pick._name, active_id=pick.id
        ...
        self.assertTrue(
            invoice.purchase_id, "Relation purchase_id in invoice are missing."
        )
        for line in invoice.picking_ids:
            self.assertEquals(
                line.id, pick.id, "Relation between invoice and picking are missing."
            )
        ...
        self.assertTrue(invoice.tax_line_ids, "Total of Taxes in Invoice are missing.")
        self.assertTrue(
