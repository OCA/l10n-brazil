...
class FinanLancamentoReceberManual(test_common.SingleTransactionCase):
    ...
    def setUp(self):
        ...
        self.finan_lancamento_a_receber = self.finan_lancamento.create({
            ...
            'numero':3567,
            'data_documento':fields.Datetime.from_string('2016-03-21 00:00:00'),
            'conta_id':self.env.ref('finan.financial_account_101001').id,
            'data_vencimento':fields.Datetime.from_string('2016-03-21 00:00:00'),
        ...
        self.finan_lancamento_recebimento = self.finan_lancamento.create({
            ...
            'documento_id':self.env.ref('finan.DOCUMENTO_FINANCEIRO_BOLETO').id,
            'numero':3567,
            'data_credito_debito':fields.Datetime.from_string('2016-03-21 00:00:00'),
            'conta_id':self.env.ref('finan.financial_account_101001').id,
            'data_pagamento':fields.Datetime.from_string('2016-03-21 00:00:00'),
            'forma_pagamento_id':self.env.ref('finan.finan_forma_pagamento_001').id,
            'banco_id':self.env.ref('finan.finan_banco_237').id,
            'vr_documento':265.92,
            'tipo':'recebimento',
    ...
    def teste3_saldo_divida_zerado(self):
        """"
        Teste 3:
        O saldo após a divida ter recebimento deve ser zerado
        """
