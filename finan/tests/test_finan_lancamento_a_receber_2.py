...
class FinanLancamentoReceberManual(test_common.SingleTransactionCase):
    ...
    def setUp(self):
        ...
        self.finan_lancamento_a_receber = self.finan_lancamento.create({
            ...
            'participante_id':self.env.ref('l10n_br_base.res_partner_akretion').id,
            'documento_id':self.env.ref('finan.DOCUMENTO_FINANCEIRO_BOLETO').id,
            'numero':8763,
            'data_documento':fields.Datetime.from_string('2018-01-20 00:00:00'),
            'conta_id':self.env.ref('finan.financial_account_101001').id,
            'data_vencimento':fields.Datetime.from_string('2018-01-20 00:00:00'),
        ...
        self.finan_lancamento_recebimento = self.finan_lancamento.create({
            ...
            'documento_id':self.env.ref('finan.DOCUMENTO_FINANCEIRO_BOLETO').id,
            'numero':8763,
            'data_credito_debito':fields.Datetime.from_string('2018-01-20 00:00:00'),
            'conta_id':self.env.ref('finan.financial_account_101001').id,
            'data_pagamento':fields.Datetime.from_string('2018-01-20 00:00:00'),
            'forma_pagamento_id':self.env.ref('finan.finan_forma_pagamento_001').id,
            'banco_id':self.env.ref('finan.finan_banco_001').id,
            'vr_documento':85.00,
            'tipo':'recebimento',
    ...
    def teste1_lancamento_a_receber_state(self):
        ...
        """"
        Teste 1:
        O state de uma conta a receber deve ser efetivo(open)
    ...
    def teste2_lancamento_recebimento_state(self):
        ...
        """"
        Teste 2:
        O state de um recebimento deve ser quitado(paid)
        e o provisorio dele, falso
        """
    ...
    def teste3_saldo_divida_zerado(self):
        ...
        """"
        Teste 3:
        O saldo após a divida ter recebimento deve ser zerado considerando
        o desconto recebido
        """

        self.finan_lancamento_recebimento.divida_id = self.finan_lancamento_a_receber.id
    ...
    def teste4_a_receber_state(self):
        ...
        """"
        Teste 4:
        Após o recebimento correto, a dívida tem quer ter state quitado(paid)
        """
