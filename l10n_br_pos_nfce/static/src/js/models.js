...
import { ChaveEdoc } from "@l10n_br_pos_nfce/js/utils";
import {
    NFeXML,
    BRAZILIAN_STATES_IBGE_CODE_MAP,
} from "@l10n_br_pos_nfce/js/nfe-xml";
...
const L10nBrNfceOrder = (Order) =>
    class L10nBrNfceOrder extends Order {
        ...
        _buildNFCeAdditionalInfo() {  //mudanças aqui para o padrao do odoo16 e tambem padrao da propria adaptacao
        return {
            ...
            authorization_date_string:
                this.authorization_date_string,

            document_date_string:
                this.document_date_string,
        ...
        get_total_icms() {
            ...
            for (const line of orderlines) {
                ...
                if (
                    productFiscalMap &&
                    productFiscalMap.icms_cst_code === "00"
                ) {
                    totalICMS +=
                        parseFloat(productFiscalMap.icms_value) || 0;
                }
            }
        ...
        get_total_pis() {
            ...
            for (const line of orderlines) {
                ...
                if (productFiscalMap) {
                    totalPIS +=
                        parseFloat(productFiscalMap.pis_value) || 0;
                }
            }
        ...
        get_total_cofins() {
            ...
            for (const line of orderlines) {
                ...
                if (productFiscalMap) {
                    totalCofins +=
                        parseFloat(productFiscalMap.cofins_value) || 0;
                }
            }
        ...
        get_total_icms_base() {
            ...
            for (const line of orderlines) {
                ...
                if (
                    ...
                    totalICMSBase +=
                        parseFloat(productFiscalMap.icms_base) || 0;
                }
            }
...
const L10nBrNfcePosGlobalState = (PosGlobalState) =>
    class L10nBrNfcePosGlobalState extends PosGlobalState {
        ...
        async push_and_invoice_order(order) {
            if (
                this.config.simplified_document_type !== "65"
            ) {
        ...
        async _save_to_server(orders, options = {}) {
            ...
            const ordersToSync = orders.filter(
                (order) =>
                    !this.syncingOrders ||
                    !this.syncingOrders.has(order.id)
            );
            ...
            const orderIdsToSync = ordersToSync.map(
                (order) => order.id
            );
            ...
            try {
                ...
                await this._fillNFCeAdditionalInfo(
                    serverIds
                );
        ...
        async _fillNFCeAdditionalInfo(orders) {
            ...
            if (!orderOption.account_move) {
                console.warn(
                    "NFC-e: create_from_ui não retornou account_move."
                );
                return;
            }
            ...
            if (move.fiscal_document_id) {
                ...
                console.log(
                    "NFC-e FISCAL DOCUMENT:",
                    fiscalDocuments
                );

                if (
                    fiscalDocuments.length &&
                    fiscalDocuments[0].nfe40_infNFeSupl
                ) {
                    const supplementId =
                        fiscalDocuments[0].nfe40_infNFeSupl[0];
        ...
        _contingenciaNFCe(orders) {
            const state =
                this.company.state_id
                    ? this.company.state_id[1]
                    : "";

            const cnpj =
                (this.company.cnpj_cpf || "")
                    .replace(/\D/g, "");
            ...
            const serie =
                this.config.nfce_document_serie_code
                    ? this.config.nfce_document_serie_code
                          .toString()
                          .padStart(3, "0")
                    : "001";
            ...
            for (const order of orders) {
                ...
                if (!currentOrder.document_number) {
                    console.warn(
                        "NFC-e em contingência: número ainda não disponível."
                    );
                    continue;
                }
                ...
                currentOrder.document_key =
                    chaveEdoc.generatedChave;

                currentOrder.document_date_string =
                    currentDate.toLocaleString();

                this._qrCodeNFCeContingency(
                    currentOrder,
                    chaveEdoc
                );
            }
        ...
        async _qrCodeNFCeContingency(
            currentOrder,
            chaveEdoc
        ) {
            const xml = new NFeXML(
                this,
                currentOrder,
                chaveEdoc
            );
            ...
            currentOrder.qr_code =
                await xml.generateQRCodeText();
        }
    };

Registries.Model.extend(
    PosGlobalState,
    L10nBrNfcePosGlobalState
);