...
class TestNFCePosOrderCommon(TestPoSCommon):
    ...
    def setUpClass(cls, **kw):
        ...
        # Disable POS unprocessed-order capture during tests.
        # This prevents _handle_order_process_fail() from masking
        # the original exception with a MissingError on pos.session.
        cls.env["ir.config_parameter"].sudo().set_param(
            "point_of_sale.capture_unprocessed_order",
            "False",
        )
        ...
        serie = cls.env["l10n_br_fiscal.document.serie"].create(
            {
                "code": "2",
                "name": "Série NFCe Teste",
                "document_type_id": cls.env.ref("l10n_br_fiscal.document_65").id,
                "company_id": cls.company.id,
            }
        )

        serie.internal_sequence_id.number_next = 1000
    ...
    def _apply_monkey_patches(cls):
        ...
        def _form_setattr__(self, field, value):
            ...
            if descr["type"] == "many2one":
                ...
                # Store just the id: that's the output of
                # default_get & (more or less) onchange.
                value = value.id
        ...
        @classmethod
        def setUpClass(cls, **kw):
            superSetUpClass.__func__(cls, **kw)
            ...
            cls.env.user.write(
                {
                    "groups_id": [
                        (
                            4,
                            cls.env.ref("l10n_br_fiscal.group_manager").id,
                        )
                    ]
                }
            )
        ...
        def setup_company_data(
            ...
            **kwargs,
            ...
            company_data = super_setup_company_data.__func__(
                cls,
                company_name,
