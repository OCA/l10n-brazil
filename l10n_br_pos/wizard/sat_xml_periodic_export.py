    ...
    def export(self):
        ...
        if pos_orders:
            ...
            for pos_order in pos_orders:
                fp_new = open(path_exportacao + pos_order.chave_cfe + ".xml", "w")
                ...
                if pos_order.cfe_cancelamento_return:
                    fp_new = open(
                        path_exportacao + pos_order.chave_cfe_cancelamento + ".xml", "w"
                    )
                    fp_new.write(base64.b64decode(pos_order.cfe_cancelamento_return))
                    fp_new.close()

                    caminhos_xmls += (
                        path_exportacao + pos_order.chave_cfe_cancelamento + ".xml "
                    )
            ...
            if not self.create_uid.company_id.parent_id.id:
                os.system(
                    "zip -r "
                    + os.path.join(
                        path_exportacao, "cfes_xmls_" + time.strftime("%Y-%m-%d")
                    )
                    + " "
                    + caminhos_xmls
                )
                ...
                os.system(
                    ...
                    + os.path.join(
                        path_exportacao,
                        "cfes_xmls_"
                        + self.create_uid.company_id.name.replace(" ", "")
                        + "_"
                        + time.strftime("%Y-%m-%d"),
                    )
                    + " "
            ...
            for pos_order in pos_orders:
                os.remove(path_exportacao + pos_order.chave_cfe + ".xml")
