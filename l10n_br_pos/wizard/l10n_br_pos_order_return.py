    ...
    def create_returns(self):
        if self.env.context.get("pos_order_id"):
            ...
            for product_line in self.product_return_moves:
                for line in pos_order.lines:
                    if line.product_id == product_line.product_id:
                        if (
                            line.qtd_produtos_devolvidos + product_line.quantity
                            > line.qty
                        ):
                            raise Warning(
                                _(
                                    "Esta quantidade do produto %s não pode "
                                    "ser devolvida"
                                )
            ...
            kwargs = {
                ...
                "partner_shipping_id": picking_devolucao.company_id.partner_id.id,
            ...
            valor_total_devolucao = self._buscar_valor_total_devolucao(pos_order)
    ...
    def _check_picking_parameters(order):
        if not order.picking_id.fiscal_operation_id:
            order.picking_id.fiscal_operation_id = (
                ...
                or order.company_id.out_pos_fiscal_operation_id
            )
        if not order.picking_id.fiscal_operation_id.refund_fiscal_operation_id:
            order.picking_id.fiscal_operation_id.refund_fiscal_operation_id = (
                order.session_id.config_id.refund_pos_fiscal_operation_id
                or order.company_id.refund_pos_fiscal_operation_id
            )
